...
for lid in [0,1]:  # lensed or unlensed - trial classification b/w 2 labels.
    ...
    for ind in range(len(fileInData)):
        ...
        if np.isnan(pixel[0].data).any(): print lid, ind, ' -- ERROR: NaN'
        print ind
        ...
        if ind in forTest:
            ...
            trainind+=1
            print (pixel[0].data).min(), (pixel[0].data).max(), labels+ str(trainind)
